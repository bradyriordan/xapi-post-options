...
if (is_admin()) {
	...
	wp_enqueue_style('style', plugin_dir_url(__FILE__) . '/includes/css/style.css', array(), null, 'screen');
		...
		if ($statement_trigger == 'page-view') {
			...
			<script type="text/javascript">
				window.addEventListener('load', send_real_xapi_statement());
					...
					?>
			<script type="text/javascript">
				document.getElementById("<?php echo $element_id ?>").addEventListener("<?php echo $dom_event ?>", function() {
					send_real_xapi_statement()
				});
			</script>
		<?php
	}
